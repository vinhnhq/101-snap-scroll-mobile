...
export default function Home() {
	...
	useEffect(() => {
			...
			(entries) => {
				for (const entry of entries) {
					if (entry.isIntersecting) {
						const id = Number((entry.target as HTMLElement).dataset.slideId);
						startTransition(() => setActiveSlide(id));
					}
				}
			},
		...
		const slides = container.querySelectorAll<HTMLElement>('[data-slide-id]');
	...
	return (
		...
		>
			...
			>
				<ViewTransition
					key={activeSlide === 1 ? 'active-1' : 'inactive-1'}
					enter="slide-up"
					default="none"
				>
					<div>
			...
			>
				<ViewTransition
					key={activeSlide === 2 ? 'active-2' : 'inactive-2'}
					enter="slide-up"
					default="none"
				>
					<div>
			...
			>
				<ViewTransition
					key={activeSlide === 3 ? 'active-3' : 'inactive-3'}
					enter="slide-up"
					default="none"
				>
					<div>
			...
			>
				<ViewTransition
					key={activeSlide === 4 ? 'active-4' : 'inactive-4'}
					enter="slide-up"
					default="none"
				>
					<div>
			...
			>
				<ViewTransition
					key={activeSlide === 5 ? 'active-5' : 'inactive-5'}
					enter="slide-up"
					default="none"
				>
					<div>
